...
**Data model** — `RadioStation` is a plain data class; the full station list is a hardcoded top-level `val RADIO_STATIONS` in `RadioStation.kt`.
...
| File | Purpose |
|------|---------|
| `RadioStation.kt` | Data class + hardcoded station list — edit here to add/remove stations |
