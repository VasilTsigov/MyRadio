...
actor StationRepository {
    ...
    // API returns these under different names — skip them since we have working hardcoded versions
    private let blockedNameFragments = ["horizont", "хоризонт", "radio 1"]
    ...
    private func parseStations(_ json: [[String: Any]], existing: [RadioStation]) -> [RadioStation] {
        ...
        for obj in json {
            ...
            if blockedNameFragments.contains(where: { lower.contains($0) }) { continue }
